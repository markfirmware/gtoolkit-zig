...
fn renderMemberJson(gpa: *Allocator, ais: *Ais, tree: ast.Tree, decl: ast.Node.Index, space: Space) Error!void {
    ...
    switch (tree.nodes.items(.tag)[decl]) {
        .fn_decl => {
            ...
            try renderExpressionJson(gpa, ais, tree, fn_proto, .space);
            var finalValue = renderExpressionJson(gpa, ais, tree, datas[decl].rhs, space);
            try endObject(ais);
            return finalValue;
        },
        .fn_proto_simple,
...
fn renderExpressionJson(gpa: *Allocator, ais: *Ais, tree: ast.Tree, node: ast.Node.Index, space: Space) Error!void {
    try startIt(ais);
    const token_tags = tree.tokens.items(.tag);
    const main_tokens = tree.nodes.items(.main_token);
    ...
    switch (node_tags[node]) {
        ...
        .anyframe_literal,
        .string_literal,
        => return renderTokenJsonIt(ais, tree, main_tokens[node], space),
        ...
        .error_value => {
            ...
            return renderTokenJsonIt(ais, tree, main_tokens[node] + 2, space);
        },

        .@"anytype" => return renderTokenJsonIt(ais, tree, main_tokens[node], space),
        ...
        .@"suspend" => {
            ...
            if (body != 0) {
                ...
                return renderTokenJsonIt(ais, tree, suspend_token, space);
            }
        },
        ...
        .field_access => {
            ...
            return renderTokenJsonIt(ais, tree, field_access.rhs, space);
        },
        ...
        .array_access => {
            ...
            ais.popIndent();
            return renderTokenJsonIt(ais, tree, rbracket, space); // ]
        },
        ...
        .unwrap_optional => {
            try renderExpression(gpa, ais, tree, datas[node].lhs, .none);
            try renderToken(ais, tree, main_tokens[node], .none);
            return renderTokenJsonIt(ais, tree, datas[node].rhs, space);
        },
        ...
        .@"continue" => {
            ...
            if (label != 0) {
                ...
                return renderTokenJsonIt(ais, tree, label, space); // label
            } else {
                return renderTokenJsonIt(ais, tree, main_token, space); // continue
            }
        },
        ...
        .grouped_expression => {
            ...
            ais.pushIndentOneShot();
            try renderExpression(gpa, ais, tree, datas[node].lhs, .none);
            return renderTokenJsonIt(ais, tree, datas[node].rhs, space); // rparen
        },
        ...
        .error_set_decl => {
            ...
            if (lbrace + 1 == rbrace) {
                ...
                return renderTokenJsonIt(ais, tree, rbrace, space);
        ...
        .anyframe_type => {
            ...
            if (datas[node].rhs != 0) {
                ...
                return renderTokenJsonIt(ais, tree, main_token, space); // anyframe
            }
        },
        ...
        => {
            ...
            ais.popIndent();
            return renderTokenJsonIt(ais, tree, tree.lastToken(node), space); // rbrace
        },
        ...
        .enum_literal => {
            try renderToken(ais, tree, main_tokens[node] - 1, .none); // .
            return renderTokenJsonIt(ais, tree, main_tokens[node], space); // name
        },
...
fn startIt(ais: *Ais) Error!void {
    try ais.writer().writeAll("<");
}

fn endIt(ais: *Ais) Error!void {
    try ais.writer().writeAll(">,");
}

fn renderTokenJsonIt(ais: *Ais, tree: ast.Tree, token_index: ast.TokenIndex, space: Space) Error!void {
    const token_tags = tree.tokens.items(.tag);
    const token_starts = tree.tokens.items(.start);
